...
namespace ManejoPresupuesto.Servicios
{
    ...
    public class ServicioReportes : IServicioReportes
    {
        ...
        public async Task<ReporteTransaccionesDetalladas> ObtenerReporteTransaccionesDetalladasPorCuenta(int usuarioId, int cuentaId, 
            int mes , int año, dynamic ViewBag)
        { //Dynamic es el tipo de dato que se usa para el ViewBag
        ...
        {
            ViewBag.mesAnterior = fechaInicio.AddMonths(-1).Month; // contenedor dinámico que vive durante la petición del controlador hacia la vista
            ViewBag.añoAnterior = fechaInicio.AddMonths(-1).Year;
            ViewBag.mesPosterior = fechaInicio.AddMonths(1).Month;
            ViewBag.añoPosterior = fechaInicio.AddMonths(1).Year;

            //obtiene la url en la que me encuentro
            //HttpContext.Request.Path -> ruta sin parametros /Cuntas/Detalle
            //HttpContext.Request.QueryString -> los parametros ?id=5
        ...
        private static ReporteTransaccionesDetalladas GenerarReporteTransaccionesDetalladas(DateTime fechaInicio, DateTime fechaFin, IEnumerable<Transaccion> transacciones)
        {
            ...
            var transaccionesPorFecha = transacciones.OrderByDescending(
                x => x.FechaTransaccion)
